...
export class UsuarioService {
  ...
  enviar_duda(formulario:any){
    const ruta = 'http://127.0.0.1:8080/usuario/nueva_pregunta';
    return this.http.post(ruta, formulario, { headers: { 'Content-Type': 'application/json' } });
  }

  respuesta_correcta(formulario:any){
    const ruta = 'http://127.0.0.1:8080/usuario/respuesta_correcta';
    return this.http.post(ruta, formulario, { headers: { 'Content-Type': 'application/json' } });
  }

  dudas(){
    const ruta = 'http://127.0.0.1:8080/usuario/dudas';
